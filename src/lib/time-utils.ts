...
// Convert Date or string to a standard Date object
// Supabase uses ISO strings, so we handle Date and string types
export function toDate(value: Date | string | null | undefined): Date | null {
    if (!value) return null;
    if (value instanceof Date) return value;
    // Handle string values (ISO strings from Supabase)
