...
export async function sendBossReport(input: BossReportInput): Promise<void> {
  const { bosses, webhookUrl } = BossReportInputSchema.parse(input);
  ...
  const messagePayloads = chunkedBosses.map((chunk, index) => {
    const embed = {
      title: index === 0 ? 'Boss Spawn Report' : undefined,
  ...
  for (let i = 0; i < messagePayloads.length; i++) {
    try {
      ...
      const errorMessage = `An unexpected error occurred while sending the Discord report: ${
